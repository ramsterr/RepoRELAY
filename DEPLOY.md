...
- `Dockerfile.api` — uses Gemini API for embeddings (zero local RAM), no model download needed.
...
**Keepalive:**
- `.github/workflows/ping.yml` — now also hits `/recommend?repo=fastapi/fastapi&limit=1&tags=python` so the Gemini API client and DB connection stay warm between pings, not just on first real user request.
...
2. First deploy after Render cold start takes ~2-3 seconds for Gemini API warmup.
3. Watch the logs for `embedding model preloaded and ready` — that's how you know the Gemini API client is initialized.
...
```
Should return 200 in **under 1 second** on a warm container, **under 5 seconds** on a cold one.
...
| Render build fails on `sentence_transformers` import | The `RUN python -c ...` step needs the package installed first. Make sure `uv sync` runs before the model download in the Dockerfile. For Gemini mode, set `EMBEDDING_API=gemini` and the import is skipped. |
| Model still downloads on cold start | `HF_HOME` not set in the running container. Check the deployed env vars in Render. With Gemini mode, no model download is needed — just set `EMBEDDING_API=gemini` + `GEMINI_API_KEY`. |
